...
namespace Toolbelt
{
    ...
    {       
        public static sbyte[] ConvertHashToSByteArray(byte[] hash)
        {
            sbyte[] shash = new sbyte[hash.Length];
            for (int i = 0; i < hash.Length; i++)
            {
                shash[i] = unchecked((sbyte)hash[i]);
            }
            return shash;
        }

        public static byte[] subkey = new byte[4168] {            
        ...
        public static UInt32[] BlowfishInitialize(byte[] keyhash, UInt16 keybytes, ref UInt32[] P, ref UInt32[] S)
        {
            ...
            const int N = 16;
            Buffer.BlockCopy(subkey, 0, P, 0, 72);
            Buffer.BlockCopy(subkey, 72, S, 0, 4096);
            ...
            for (i = 0; i < N + 2; i += 2)
            {
                BlowfishEncipher(ref datal, ref datar, P, S);
        ...
        public static byte[] SetupHashKey(byte[] hashkey)
        {
            byte[] key;
            using (var md5 = MD5.Create())
            {
                md5.TransformFinalBlock(hashkey, 0, 20);
                key = md5.Hash;
            }
            return key;
        }
